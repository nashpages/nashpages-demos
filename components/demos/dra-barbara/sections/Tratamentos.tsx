  ...
  return (
    ...
    >
      <div className="max-w-[1280px] mx-auto">
        ...
        {/* Desktop: 3 cards inline staggered */}
        <Stagger delay={0.35} staggerChildren={0.15} className="hidden lg:grid grid-cols-3 gap-16 mt-20">
          {t.cards.map((card) => (
            <StaggerItem key={card.num}>
              <article className="group">
                <p
                  className="font-normal text-[var(--c-nude)] tracking-[-0.03em] leading-[1] text-[110px] transition-transform duration-700 group-hover:-translate-y-1"
                  style={{ fontFamily: "var(--font-playfair)" }}
                >
                  {card.num}
                </p>
        ...
        {/* Mobile: stack */}
        <Stagger delay={0.35} staggerChildren={0.15} className="lg:hidden mt-12 space-y-10 divide-y divide-[var(--c-linha)]">
          {t.cards.map((card, i) => (
            <StaggerItem key={card.num}>
              <article className={i > 0 ? "pt-10" : ""}>
                <p
